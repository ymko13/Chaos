...
{    
  ...
  //  [...]
	VSmartPtr<mCharacter> s = (mCharacter *) Vision::Game.SearchEntity("Player");
	VASSERT(s);
	mEffect effect = mEffect();
	effect.SetTimeSpan(10.f);
	effect.SetValue(0.5f);
	effect.SetType(ET_Debuff);
	effect.SetDebuff(ED_Freeze);
	s->ApplyEffect(effect);

	//for(int i = 1; i < 11; i++)
	//{
	//	hkvVec3 pos(0);
	//	VSmartPtr<mProjectile> s = (mProjectile *) Vision::Game.CreateEntity("mProjectile", pos);
	//	s->SetProjectileMesh("Models/Sphere.model");
	//	s->SetProjectileSpeed(1200);
	//	s->SetProjectileTargetPosition(hkvVec3(5000 / i,500 * i,0));
	//	s->SetAlive(true);
	//}

	Print ( "========================================" );
	Print ( "========== LAUNCHED A BULLET! ==========" );
	Print ( "========================================" );
